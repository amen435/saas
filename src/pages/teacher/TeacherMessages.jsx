...
export default function TeacherMessages() {
  ...
  const { data: classesRes = {} } = useQuery({
    queryKey: ["teacherAnnouncementClasses"],
  ...
  useEffect(() => {
    if (isHomeroom && !announcementForm.classId) {
      const own = classOptions.find((c) => c.isHomeroom) || classOptions[0];
      if (own?.id) {
        setAnnouncementForm((prev) => ({ ...prev, classId: String(own.id), audienceType: "CLASS" }));
      }
    }
  }, [isHomeroom, classOptions, announcementForm.classId]);
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setAnnouncementForm({ title: "", message: "", audienceType: "CLASS", classId: "", expiryDate: "" });
  ...
  const handleAnnouncement = () => {
    ...
    if (!announcementForm.classId) { toast.error("Class is required"); return; }
    const payload = {
      title: announcementForm.title,
      message: announcementForm.message,
      targetRole: "ALL",
      audienceType: "CLASS",
      classId: Number(announcementForm.classId),
      expiryDate: announcementForm.expiryDate || null,
    };
    if (editingAnnouncement) {
      updateMutation.mutate({ id: editingAnnouncement.announcementId, payload });
    } else {
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Communicate with parents about student progress</p>
        </div>
        <Button onClick={() => setAnnouncementDialog(true)} className="gradient-primary text-primary-foreground gap-2">
          <Bell className="w-4 h-4" /> Class Announcement
        </Button>
      </div>
      ...
      {tab === "messages" ? (
        <DirectMessagingPanel
          emptyHint="No parent contacts yet. Parents appear here when they are linked to students in classes you teach or homeroom."
                    ...
                    <button
                      onClick={() => {
                        ...
                        setAnnouncementForm({
                          title: a.title,
                          message: a.message,
                          audienceType: a.audienceType || "CLASS",
      ...
      <Dialog open={announcementDialog} onOpenChange={setAnnouncementDialog}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Class Announcement</DialogTitle>
            <DialogDescription>Send an announcement to parents of your class.</DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleAnnouncement} className="gradient-primary text-primary-foreground">{editingAnnouncement ? "Update" : "Send Announcement"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>

...
import { User, Mail, Phone, School, Calendar, Lock, Camera, Bell, Globe, BookOpen, Star, Sun, Moon, Monitor, Award } from "lucide-react";
...
export default function TeacherProfile() {
  ...
  const [notifications, setNotifications] = useState({
    email: true, homework: true, grades: true, meetings: true, daily: false,
  });
  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        ...
        console.log("profile response:", detailRes?.data ?? detailRes);
  ...
  const displayName = profile?.user?.fullName || profileSummary?.fullName || user?.fullName || user?.name || "Teacher";
  const displayId = profile?.user?.userId || profile?.teacherId || profileSummary?.userId || user?.userId || "-";
  ...
  const subjects = useMemo(() => {
    const s = new Set();
    profile?.classTeachers?.forEach?.((ct) => ct?.subjectName && s.add(ct.subjectName));
    return Array.from(s);
  }, [profile]);
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      <motion.div {...anim} transition={{ delay: 0.15 }} className="bg-card rounded-xl border border-border p-6">
        ...
        <div className="space-y-3">
          {[
            ...
            { key: "homework", label: "Homework submission alerts", icon: BookOpen },
            { key: "grades", label: "Grade update alerts", icon: Star },
            { key: "meetings", label: "Parent meeting reminders", icon: Bell },
            { key: "daily", label: "Daily class summary", icon: Calendar },
          ].map((n) => (
      ...
      <motion.div {...anim} transition={{ delay: 0.2 }} className="bg-card rounded-xl border border-border p-6">
        ...
        <p className="text-xs text-text-secondary">Achievements are not connected yet.</p>
      </motion.div>
    </div>
  );
}

...
export default function ParentGrades() {
  ...
  const {
    ...
    queryFn: async () => {
      if (import.meta?.env?.DEV) console.debug("[ParentGrades] GET /parents/me/children");
      const res = await api.get("/parents/me/children");
      if (import.meta?.env?.DEV) console.debug("[ParentGrades] response.data (children)", res);
  ...
  const {
    ...
    queryKey: ["studentSummary", selectedChild, effectiveAcademicYear, semester],
    queryFn: async () => {
      const response = await gradeService.getStudentSummary(selectedChild, {
  ...
  const subjects = Array.isArray(gradeSummary?.subjects) ? gradeSummary.subjects : [];
  const classRank = gradeSummary?.classRank ?? gradeSummary?.overallRank?.classRank ?? gradeSummary?.overallRank?.rank ?? null;
  const classRankTotal = gradeSummary?.overallRank?.totalStudents ?? null;
  const schoolRank = gradeSummary?.schoolRankValue ?? gradeSummary?.schoolRank?.schoolRank ?? gradeSummary?.schoolRank?.rank ?? null;
  const schoolRankTotal = gradeSummary?.schoolRank?.totalStudents ?? null;

  const total = subjects.reduce((sum, s) => sum + Number(s?.average ?? 0), 0);
  const maxTotal = subjects.length * 100;
  const overallAvg =
    typeof gradeSummary?.summary?.average === "number"
      ? Math.round(gradeSummary.summary.average)
      : subjects.length
  ...
  return (
    <div className="space-y-6" ref={printRef}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-heading">Grade Center</h1>
          <p className="text-sm text-text-secondary">Academic Year {effectiveAcademicYear} · {semesterLabel}</p>
        </div>

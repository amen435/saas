...
import { User, Mail, Phone, School, Calendar, Lock, Camera, Bell, Globe, BookOpen, Sun, Moon, Monitor } from "lucide-react";
...
export default function StudentProfile() {
  ...
  const [notifications, setNotifications] = useState({
    email: true, homework: true, grades: true, aiTips: true, daily: false,
  });
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        ...
        console.log("profile response:", studentResponse?.data ?? studentResponse);
  ...
  const displayName = profile?.fullName || profileSummary?.fullName || user?.fullName || user?.name || "Student";
  const displayId = profile?.studentId || profileSummary?.userId || user?.studentId || user?.userId || "-";
  const displayEmail = profileSummary?.email || user?.email || "-";
  const displayPhone = profileSummary?.phone || user?.phone || "-";
  const className = profile?.className || "-";
  const schoolName = profile?.schoolName || profileSummary?.school?.schoolName || user?.schoolName || "-";
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      <motion.div {...anim} className="bg-card rounded-xl border border-border p-6">
        <div className="flex flex-col sm:flex-row items-center gap-5">
          ...
          <div className="text-center sm:text-left">
            ...
            <p className="text-xs text-text-secondary">{className} | {schoolName}</p>
        ...
        <motion.div {...anim} transition={{ delay: 0.05 }} className="bg-card rounded-xl border border-border p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {[
              ...
              { icon: School, label: "Student ID", value: displayId },
              { icon: School, label: "Class", value: className },
              { icon: BookOpen, label: "Academic Year", value: academicYear },
      ...
      <motion.div {...anim} transition={{ delay: 0.1 }} className="bg-card rounded-xl border border-border p-6">
        <h3 className="font-semibold text-heading mb-4 flex items-center gap-2">
          <Bell className="w-4 h-4 text-muted-foreground" /> Preferences
        </h3>
        ...
        <div className="space-y-3">
          {[
            ...
            { key: "homework", label: "Homework reminders", icon: BookOpen },
            { key: "grades", label: "Grade alerts", icon: User },
            { key: "aiTips", label: "AI tutor tips", icon: Globe },
